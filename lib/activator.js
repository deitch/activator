	...
	if (!id) {
		...
		async.waterfall([
			...
			function(res,cb) {
				...
				mailer("activate",req.lang||"en_US",{code:code,email:email,id:id,request:req},from,email,cb);
			}
		],function (err) {
			var code = 400;
			if (err) {
...
createPasswordReset = function (req,done) {
	...
	async.waterfall([
		...
		function(res,cb) {
			...
			mailer("passwordreset",req.lang||"en_US",{code:reset_code,email:email,id:id,request:req},from,email,cb);
		}
	],function (err) {
		var code = 400;
		if (err) {
